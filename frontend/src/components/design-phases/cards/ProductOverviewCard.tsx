    ...
    <Card className="border-gray-200 dark:border-gray-700 shadow-sm">
      ...
      <CardContent className="space-y-4">
        ...
        {overview.problems.length > 0 && (
          <details
            open={problemsOpen}
            onToggle={(e) => setProblemsOpen((e.target as HTMLDetailsElement).open)}
          >
            <summary className="flex items-center justify-between w-full py-2 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
        ...
        {overview.features.length > 0 && (
          <details
            open={featuresOpen}
            onToggle={(e) => setFeaturesOpen((e.target as HTMLDetailsElement).open)}
          >
            <summary className="flex items-center justify-between w-full py-2 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                Key Features
