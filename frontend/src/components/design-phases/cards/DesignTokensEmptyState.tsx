...
interface DesignTokensEmptyStateProps {
  onDefineWithAI?: () => void
  disabled?: boolean
}

export function DesignTokensEmptyState({ onDefineWithAI, disabled }: DesignTokensEmptyStateProps) {
  return (
      ...
      <CardContent className="py-8">
        <div className="flex flex-col items-center text-center max-w-sm mx-auto">
          <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center mb-3">
            <Palette className="w-5 h-5 text-gray-400 dark:text-gray-500" strokeWidth={1.5} />
          </div>
          <h3 className="text-base font-medium text-gray-600 dark:text-gray-400 mb-1">
            No design tokens defined yet
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Use AI to choose colors, typography, and design tokens for your app.
          </p>
          {onDefineWithAI && (
