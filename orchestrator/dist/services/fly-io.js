...
class FlyIOService {
     ...
     */
    async verifyMachineDestroyed(machineId) {
        try {
            const machine = await this.getMachine(machineId);
     ...
     */
    async findContainersForProject(projectId) {
        try {
            const machines = await this.listMachines();
            return machines.filter(machine => machine.config?.metadata?.['velocity-project-id'] === projectId &&
     ...
     */
    async cleanupProjectContainers(projectId) {
        try {
